...
def modify_backprop(model, name, model_generator, weights=None):
    ...
    with g.gradient_override_map({'Relu': name}):
        ...
        if weights:
            new_model = model_generator(weights)
        else:
...
def grad_cam(input_model, image, category_index, layer_name, nb_classes, size, original_im=None):
    model = Sequential()
    ...
    #output, grads_val = gradient_function([image, 0])       # theano
    output, grads_val = output[0, :], grads_val[0, :, :, :]
    #output, grads_val = output[0], grads_val                # theano

    weights = np.mean(grads_val, axis = (0, 1)) #(1,2)
...
def general_gradCAM(model, preprocessed_input, predicted_class, model_generator, weights=None, original_im=None, block_CAM="block5_conv3", nb_classes = 1000, size=224):
    ...
    saliency_fn = compile_saliency_function(guided_model, block_CAM)
    saliency = saliency_fn([preprocessed_input, 0])
    gradcam = saliency[0] * heatmap[..., np.newaxis]
    deproced_im = deprocess_image(gradcam)
    cv2.imwrite("guided_gradcam.jpg", deproced_im)
    """
    #K.clear_session()
    return cam
